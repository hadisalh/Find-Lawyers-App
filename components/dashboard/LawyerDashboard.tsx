...
export const LawyerDashboard: React.FC<LawyerDashboardProps> = ({
  ...
  const renderContent = () => {
    switch(activeView) {
      case 'posts':
        return (
          <>
            <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-slate-100 mb-6">استشارات العملاء المتاحة</h2>
            {posts.length > 0 ? (
              posts.map(post => (
                <PostCard
                  key={post.id}
              ...
              <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-lg shadow-md border dark:border-slate-700">
                  <p className="text-slate-600 dark:text-slate-300 text-lg">لا توجد استشارات منشورة حاليًا.</p>
              </div>
            )}
          </>
        );
                    ...
                    {lawyerChats.map(chat => {
                      ...
                      return (
                        <li key={chat.id} className="p-4 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer flex items-center gap-4" onClick={() => onChatIconClick(chat.id)}>
                          <div className="bg-slate-200 dark:bg-slate-600 p-3 rounded-full">
                          ...
                          <div className="flex-grow">
                           ...
                           <div className="text-xs text-slate-500 dark:text-slate-400 ml-auto whitespace-nowrap">
  ...
  return (
    <DashboardLayout sidebar={sidebarContent}>
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-slate-100">أهلاً بعودتك، {currentUser.fullName}</h1>
        <p className="text-md text-slate-500 dark:text-slate-400">هنا ملخص نشاطك وأحدث الاستشارات المتاحة.</p>
      </div>
