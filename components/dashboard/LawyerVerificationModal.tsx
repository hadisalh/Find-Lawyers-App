  ...
  const InfoRow: React.FC<{icon: React.ReactNode; label: string; value: string}> = ({ icon, label, value }) => (
    <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
        ...
        <div>
            ...
            <p className="text-md font-bold text-slate-800 dark:text-slate-100">{value}</p>
        </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b dark:border-slate-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800 dark:text-slate-200">التحقق من بيانات المحامي</h2>
        ...
        <main className="p-6 space-y-4">
          ...
          <div>
            <a href={lawyer.idUrl} target="_blank" rel="noopener noreferrer" className="block w-full text-center p-3 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 font-bold rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900 transition-colors">
