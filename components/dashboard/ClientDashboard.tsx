        ...
        return (
          <>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800">استشاراتي ومنشوراتي</h2>
            ...
            {showAddForm && (
                 <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 mb-8">
                    <form onSubmit={(e) => {
                        ...
                        <h2 className="text-2xl font-bold text-slate-800 mb-4">أضف استشارة جديدة</h2>
                        <input name="title" placeholder="عنوان الاستشارة" className="w-full p-3 border border-slate-300 rounded-lg" required />
                        <textarea name="description" placeholder="وصف الحالة" className="w-full p-3 border border-slate-300 rounded-lg" rows={5} required />
            ...
            )) : !showAddForm && <p className="text-center text-slate-500 py-10">لا توجد منشورات خاصة بك. أضف واحدة الآن!</p>}
          </>
        );
      case 'all-posts':
        return (
            <>
                <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 mb-6">كل الاستشارات</h2>
                ...
                )) : <p className="text-center text-slate-500 py-10">لا توجد استشارات منشورة حاليًا.</p>}
            </>
        );
      case 'lawyers':
        return (
            <>
                <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 mb-6">تصفح المحامين</h2>
                <div className="bg-white p-4 rounded-xl shadow-md mb-6 sticky top-0 md:top-4 z-10 border border-slate-200">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full p-3 pr-10 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
                            />
                        </div>
                        <select value={specialtyFilter} onChange={e => setSpecialtyFilter(e.target.value as any)} className="w-full p-3 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
                    {filteredLawyers.length > 0 ? (
                        filteredLawyers.map(lawyer => (
                            <div key={lawyer.id} className="bg-white p-6 rounded-xl shadow-lg border text-center flex flex-col transition-transform hover:-translate-y-2 hover:shadow-xl">
                                <div className="bg-slate-100 p-3 rounded-full w-24 h-24 mx-auto -mt-16 border-4 border-white flex items-center justify-center">
                                    <UserIcon className="w-12 h-12 text-slate-500"/>
                                </div>
                                <h3 className="font-bold text-xl mt-4 text-slate-800">{lawyer.fullName}</h3>
                                <p className="text-emerald-600 font-semibold my-1">{lawyer.specialty}</p>
                                <div className="flex justify-center items-center gap-1 my-2 text-yellow-400">
                                    ...
                                    <span className="text-slate-600 font-bold ml-1">({lawyer.rating})</span>
                                </div>
                        ...
                        <div className="col-span-full text-center py-16 bg-white rounded-lg shadow-md border">
                            ...
                            <h3 className="mt-4 text-lg font-semibold text-slate-800">لم يتم العثور على محامين</h3>
                            <p className="mt-1 text-sm text-slate-500">جرّب تعديل مصطلحات البحث أو تغيير الفلتر.</p>
                        </div>
                    )}
                </div>
            </>
                    ...
                    {clientChats.length > 0 ? (
                        <ul className="divide-y divide-slate-200">
                            {clientChats.map(chat => {
                                ...
                                return (
                                    <li key={chat.id} className="p-3 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer flex items-center gap-4" onClick={() => onChatIconClick(chat.id)}>
                                        <div className="bg-slate-200 p-3 rounded-full"><UserIcon className="w-6 h-6 text-slate-600"/></div>
                                        <div>
                                            <p className="font-bold text-slate-800">{lawyer.fullName}</p>
                                            <p className="text-sm text-slate-500">اضغط لعرض المحادثة</p>
                                        </div>
                                    </li>
                                );
                            })}
                        </ul>
                    ) : <p className="text-center text-slate-500 py-10">لا توجد لديك محادثات.</p>}
                </div>
            </>
        );
    }
